        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                async with client.stream(
                    ...
                    print("Starting to read stream lines...", flush=True)
                    
                    async for line in response.aiter_lines():
                        line_count += 1
                        
                        # Log first 20 lines to see what we're getting
                        if line_count <= 20:
                            print(f"LINE {line_count}: {line[:200]}", flush=True)
                        ...
                        if line.startswith("data: "):
                            ...
                            if data_count <= 5:
                                print(f"DATA {data_count}: {data_str[:300]}", flush=True)
                            ...
                            try:
                                ...
                                if companies_seen <= 10 or companies_seen % 10 == 0:
                            ...
                            if data_count <= 3:
                                print(f"ID: {last_timepoint}", flush=True)
                            timepoint_file.write_text(last_timepoint)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .links a { color: #3b82f6; text-decoration: none; margin-right: 12px; font-size: 13px; }
        .copy-btn { background: #f3f4f6; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px; margin-left: 8px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="companies-table">
            <table>
                <thead>
                    <tr><th>Number</th><th>Name</th><th>SIC</th><th>Type</th><th>Time</th><th>Links</th></tr>
                </thead>
    ...
    <script>
        ...
        async function loadCompanies() {
            try {
                ...
                if (d.companies.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:40px;">No companies yet</td></tr>';
                    return;
                }
                tbody.innerHTML = d.companies.map(c => {
                    const sics = c.sic_codes.map(s => `<span class="sic-badge">${s}</span>`).join('');
                    const typeClass = c.source_type === 'target_sic' ? 'target_sic' : 'buzzword';
                    const typeLabel = c.source_type === 'target_sic' ? 'Target SIC' : 'Buzzword';
                    return `<tr class="company-row" data-number="${c.company_number}">
                        ...
                        <td>${c.company_name}</td><td>${sics}</td>
                        <td><span class="type-badge ${typeClass}">${typeLabel}</span></td>
                        <td class="time-ago">${formatTimeAgo(c.published_at)}</td>
                        <td class="links">
        ...
        function connectSSE() {
            ...
            eventSource.onerror = () => { updateStatus(false); eventSource.close(); setTimeout(connectSSE, 5000); };
        }
...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=PORT)# Buzzwords to match in company names
